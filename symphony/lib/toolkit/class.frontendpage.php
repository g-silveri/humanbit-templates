...
class FrontendPage extends XSLTPage
{
    ...
    private function __buildPage()
    {
        ...
        if (is_array($_GET) && !empty($_GET)) {
            foreach ($_GET as $key => $val) {
                ...
                //$rep[1] = '%C2%B4';
                //$rep[2] = '%5C';
                //$val = preg_replace($patterns, $rep, $val);
				$val = preg_replace('/[^A-Za-z0-9\-]/', '', $val); // Removes special chars.

                $this->_param['url-' . $key] = htmlspecialchars($val, ENT_QUOTES, 'UTF-8');
            }
        }
    ...
    {
		$vowels = array(",", "(", ")", "%", ";", ".");
		$safeValue = str_replace($vowels,"",$parameter);
        return XMLElement::stripInvalidXMLCharacters($safeValue);
    }
}
